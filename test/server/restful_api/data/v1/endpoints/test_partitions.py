...
from server.restful_api.data.v1.endpoints.partitions import PartitionsEndpoint
...
class TestPartitionsEndpoint(TestCase):
    def setUp(self):
        class MockCpucoresEndpoint(PartitionsEndpoint):
            _outbound_gate = BaseMockOutboundGate

        self.endpoint = MockCpucoresEndpoint()

        self.request = RequestHolder()
        self.request.set_uri("opserv.org/test/api/data/v1/partitions")
